...
def align_by_4_corners(image, corners, new_image_size = (1280, 768), margin_percent=10):
    ...
    # left = min(corners[0::2])
    # right = max(corners[0::2])
...
#     horiz_length = np.sqrt(pow((corners[0, 0]-corners[1, 0]),2)+pow((corners[0, 1]-corners[1, 1]),2)) + 2*margin_x
#     verti_length = np.sqrt(pow((corners[0, 0]-corners[3, 0]),2)+pow((corners[0, 1]-corners[3, 1]),2)) + 2*margin_y
#
